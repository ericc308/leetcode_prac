...
void NodeTree::sort_to_median_tree(){
  if(data.size()<2){
...
int NodeTree::is_valid_half_position(int index){
...
int NodeTree::is_right_half_tree(int index){
...
void NodeTree::exchange_to_root(int index){
  int val_root = data[1];
  int val_exchange = data[index];
  data[1] = val_exchange;
  data[index] = val_root;
}
void NodeTree::exchange_right_child(int index){
...
void NodeTree::exchange_left_child(int index){
...
void NodeTree::fix_node(int index){
...
void NodeTree::fix_node_down(int index_in){
  int length = data.size();
  int index=index_in;
  do{
    if(index*2 > length||index*2+1 > length){
...
int NodeTree::get_val(int index){
  int length = data.size();
  if(index >= length){
...
int NodeTree::return_n_sqrt_of_2(int num){
  int n=0;
  int mod = num/2;
  while(mod>0){
    n++;
